...
function Decoder(bytes, port) {
    // 验证最小帧长度
    if (bytes.length < 15) return { error: "Frame too short" };
    
    // 验证帧头帧尾
    if (bytes[0] !== 0x68) return { error: "Invalid frame start" };
    if (bytes[bytes.length - 1] !== 0x16) return { error: "Invalid frame end" };
